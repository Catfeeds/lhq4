...
class ChannelReportFormController extends ComController{
	public function channelReportForm(){
        $method = $page_no = $chan_id = $app_name = $channel_name = $start_date = $end_date = $search ='';
					...
					if ($userGroupId == '3') {
						if (empty($provider_id)) {
							...
							foreach ($appid as $k => $v) {
								//查询广告主激活回调日志
								$blogss = D('ChannelCallbackLog')->getChannelCallbackForSw($v, $provider_id,$start_date,$end_date);
						...
						if (is_array($chan_id) && empty($chan_id)) {
							...
							foreach ($appid as $k => $v) {
								//查询广告主激活回调日志
								$blogss = D('ChannelCallbackLog')->getChannelCallbackByTime($v, $chan_id,$start_date,$end_date);
				...
				switch ($userGroupId) {
					case '3':  //商务自己的
						if (empty($provider_id)) {
							...
							//查询广告主激活回调日志
							$blogs = D('ChannelCallbackLog')->getChannelCallbackForSw($app_id, $provider_id,$start_date,$end_date);
							//查询渠道商点击上报日志
					...
					default:
						if (is_array($chan_id) && empty($chan_id)) {
							...
							//   var_dump($clogs);
							$blogs = D('ChannelCallbackLog')->getChannelCallbackByTime($app_id, $chan_id,$start_date,$end_date);
			...
			switch ($userGroupId) {
				case '3':    //商务数据
					if (empty($provider_id)) {
						...
						$blogs = D('ChannelCallbackLog')->getChannelCallbackProviderId($app_id,$provider_id,$start_date,$end_date);
				...
				case '4':  //媒介数据
					if (empty($chan_id)) {
						...
						$blogs = D('ChannelCallbackLog')->getChannelCallbackByChanId($app_id,$chan_id,$start_date,$end_date);
...
// 显示分页栏
		...
		$page_html=Pagination::showPager("channelReportForm.php?app_name=$app_name&chan_id=$chan_id&start_date=$start_date&end_date=$end_date&search=1",$page_no,PAGE_SIZE,$row_count);
