...
<div class="btn-toolbar" style="margin-bottom:2px;">
    <a href="<?php echo U('pcConfig_add');?>" class="btn btn-primary"><i class="icon-plus"></i> 添加排重配置信息</a>
</div>
<form class="form_search"  action="/lhq/Backend/Pcconfig/pc_config" method="GET" style="margin-bottom:0px">
    <div style="float:left;margin-right:5px">
        <label>查询排重配置信息名称</label>
        <input type="text" name="config_name" value="<?php echo ($_GET["config_name"]); ?>" placeholder="输入排重配置信息名称" >
	...
	<div style="float:left;margin-right:5px">
		...
		<select name="provider_id" id="">
			<?php if(is_array($providers)): foreach($providers as $provider_id=>$provider): ?><option value="<?php echo ($provider_id); ?>"
					<?php if($provider_id == $_GET['provider_id']): ?>selected="selected"<?php endif; ?>
...
<div class="block">
	<a href="#page-stats" class="block-heading" data-toggle="collapse">排重配置信息</a>
	...
	<table class="table table-striped">
			...
			<?php if(is_array($configs)): foreach($configs as $key=>$config): ?><tr>
				...
				<td><?php echo ($apps[$config['app_id']]); ?></td>
				<td><?php echo ($provider_name[$appPids[$config['app_id']]]); ?></td>
				<td><?php echo ($config["request"]); ?></td>
				<td><?php echo (htmlspecialchars($config["repeat_url"])); ?></td>
				<td><?php echo ($config["config_if"]); ?></td>
			<!--	<td><?php if($config["config_type"] == '3'): ?>自身兼广告主排重
					<?php elseif($config["config_type"] == '2'): ?>
					广告主排重
					<?php elseif($config["config_type"] == '1'): ?>
					自身排重
					<?php elseif($config["config_type"] == '0'): ?>
					不排重<?php endif; ?></td>
			-->	<td>
					<a href="<?php echo U('pcConfig_modify',array('config_id'=>$config['config_id']));?>" title= "修改" ><i class="icon-pencil"></i></a>
					
					&nbsp;
					<a data-toggle="modal" href="<?php echo U('pc_config',array('config_id'=>$config['config_id'],'method'=>del));?>" title= "删除" ><i class="icon-remove"></i></a>
				</td>
