...
<div style="border:0px;padding-bottom:5px;height:auto">
    ...
    <div style="float:left;margin-right:5px">
		...
		<select name="provider_id" id="DropDownTimezone" class="input-xlarge">		
			<?php if(is_array($providers)): foreach($providers as $provider_id=>$provider): if($_GET["provider_id"] == $provider_id): ?><option value="<?php echo ($provider_id); ?>" selected><?php echo ($provider); ?></option>
					<?php else: ?>
	...
	<div style="float:left;margin-right:5px">
		...
		<select name="chanid" id="DropDownTimezone" class="input-xlarge">		
			<?php if(is_array($channels)): foreach($channels as $channel_id=>$channel): if($_GET["chanid"] == $channel_id): ?><option value="<?php echo ($channel_id); ?>" selected><?php echo ($channel); ?></option>
					<?php else: ?>
					<option value="<?php echo ($channel_id); ?>"><?php echo ($channel); ?></option><?php endif; endforeach; endif; ?>
		</select>
...
<div class="block">
	...
	<table class="table table-striped">
			...
			<tbody>							  
			<?php if(is_array($rlogs)): foreach($rlogs as $key=>$rlog): ?><tr>
				 
				<td><?php echo ($rlog["id"]); ?></td>
				
				<td><?php echo ($apps[$rlog['app_id']]); ?></td>

				<td><?php echo ($rlog['ip']); ?></td>
				 				
				<td><?php echo ($rlog["idfa"]); ?></td>

					<td><?php echo ($rlog["kid"]); ?></td>

				<td><?php echo ($channel_name[$rlog['chanid']]); ?></td>
				
				<td><?php echo ($rlog["rtime"]); ?></td>
				 
				<td><?php echo ($rlog["result"]); ?></td>
