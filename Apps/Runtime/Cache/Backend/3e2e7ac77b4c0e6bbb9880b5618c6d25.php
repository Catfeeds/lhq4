...
<div class="block">
	<a href="#page-stats" class="block-heading" data-toggle="collapse">广告供应商回调日志</a>
	...
	<table class="table table-striped">
			...
			<tbody>							  
			<?php if(is_array($plogs)): foreach($plogs as $key=>$plog): ?><tr>
				 
				<td><?php echo ($providers[$plog['provider_id']]); ?></td>
				
				<td><?php echo ($apps[$plog['app_id']]); ?></td>
				 
				<td><?php echo ($channels[$plog['chan_id']]); ?></td>
				 				
				<td><?php echo ($plog["mac"]); ?></td>
				
				<td><?php echo ($plog["idfa"]); ?></td>
				 
				<td><?php echo ($plog["time"]); ?></td>
				<td><?php echo ($plog["own_result"]); ?></td>
				<td><?php echo ($plog["info_explain"]); ?></td>
