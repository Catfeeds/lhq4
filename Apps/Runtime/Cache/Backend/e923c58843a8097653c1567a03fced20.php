...
<div class="btn-toolbar" style="margin-bottom:2px;">
    <a href="<?php echo U('providerConfig_add');?>" class="btn btn-primary"><i class="icon-plus"></i> 添加广告主配置信息</a>
</div>
<form class="form_search"  action="/lhq/Backend/Providerconfig/providerConfig" method="GET" style="margin-bottom:0px">
    <div style="float:left;margin-right:5px">
        <label>查询配置信息名称</label>
        <input type="text" name="config_name" value="<?php echo ($_GET["config_name"]); ?>" placeholder="输入配置信息名称" >
        <input type="hidden" name="search" value="1" >
    </div>
	<div style="float:left;margin-right:5px">
		<label>选择广告商</label>
		<select name="provider_id" required="true">
...
<div class="block">
	...
	<table class="table table-striped">
			...
			<?php if(is_array($configs)): foreach($configs as $key=>$config): ?><tr>
				...
				<td><?php echo (htmlspecialchars($config["config_content"])); ?></td>
				<td><?php echo ($config["config_if"]); ?></td>
				<td>
					<a href="<?php echo U('Providerconfig/providerConfig_modify',array('config_id'=>$config['config_id']));?>" title= "修改" ><i class="icon-pencil"></i></a>
					
					&nbsp;
					<a data-toggle="modal" href="<?php echo U('Providerconfig/providerConfig',array('config_id'=>$config['config_id'],'method'=>del));?>" title= "删除" ><i class="icon-remove"></i></a>
				</td>
