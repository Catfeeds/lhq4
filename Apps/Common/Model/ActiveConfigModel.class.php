...
use Think\Model;
class ActiveConfigModel extends Model {
	...
	public function getActiveConfigByApp_id($appid) {
		$sub_condition = array();
		if($appid != ''){
	...
	public function getActiveConfigById($sample_id) {
	...
	public function getActiveConfigByAppid($app_id) {
   ...
   //根据app_id查询getConfigByIfAppid
	...
	public function getActiveConfigByName($sample_name) {
		if (! $sample_name ) {
			return false;
		}
	...
	public function addActiveConfig($sample_data) {
	...
	public function delActiveConfig($sample_id) {
		if (! $sample_id || ! is_numeric ( $sample_id )) {
			return false;
		}
	...
	public function delActiveConfigByAppid($app_id) {
		if (! $app_id || ! is_numeric ( $app_id )) {
			return false;
		}
	...
	public function updateActiveConfigInfo($sample_id,$sample_data) {
		if (! $sample_data || ! is_array ( $sample_data )) {
			return false;
		}
		$condition=array("config_id"=>$sample_id);
	...
	// public function getActiveConfigsByPage($start ,$page_size) {
	// 	$db=self::__instance();
 ...
 //        $sql = "select * from ".self::getTableName()." $where order by active_config.config_id desc $limit";
 //        $list=$db->query($sql)->fetchAll();
 //        //var_dump($list);
 //        if ($list) {
 //            return $list;
 ...
 //            $limit =" limit $start,$page_size ";
 //        }
 //        $sql = "select * from ".self::getTableName()."  order by active_config.config_id desc $limit";
