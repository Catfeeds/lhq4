...
class ErrorMessage {
	...
	const SAMPLE_NOT_EXIST 	="配置信息不存在";
	const QUICKNOTE_NOT_EXIST 	="Quick note 不存在";
    const NOT_REASONABLE	="计划数量不可小于完成数量";
    const FILE_NOT_EXIT	="文件不存在";
    const file_type_not	="文件类型不符合";
    const file_same_name	="文件名已存在";
    const file_error	="文件上传出错";
    const file_type_error	="文件类型出错";
    const file_too_large	="文件太大";
    const MEMBER_NOT_EXIST	="用户不存在";
}
