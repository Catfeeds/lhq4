...
var tpl = '<tr data-id={{id}} style="text-align:center; "><td><input class="xuhao w70" style="background: none; text-align:center;" type="text" value="{{taxis}}"></input></td><td>{{id}}</td> <td>{{typename}}</td> <td>{{description}}</td>'
	+ '<td><a href="javascript:toggleGoodsType({{id}}, {{status}});" title="{{a}}当前商品类型">{{type}}</a></td>'
	+ '<td><a href="'+api.edit+'&id={{id}}">编辑</a> <a class="goods-del poi">删除</a></td></tr>';

//显示列表的div
var	$goods = $('.goods-list');

//显示数据的body体
var	$list = $goods.find('tbody');

//显示页码
var	$page = $goods.find('.page');
...
function getList(data){
	...
	$.getJSON(
			...
			api.data,		 //发送的数据
			function(jdata){		//回调函数
				$.each( jdata.goods_type , function(name, val){						
...
.on('click', '.goods-del', function(){
	...
	if(confirm('确认删除  '+ id +' -- '+ title +' 的商品类型 ?')){
...
function toggleGoodsType(id){
		...
		$.post(api.toggleGoodsType, {id: id}, function(){
		//console.log(id);
		getList();
	});
}
//修改序号
$(".goods-list").on("change",".xuhao",function(){
